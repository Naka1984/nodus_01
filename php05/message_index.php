...
//２．データ登録SQL作成
$pdo = db_conn();
$stmt = $pdo->prepare('SELECT * FROM message WHERE destination_user_name=:name');
...
<body>
  ...
  <main>
    ...
    <div><p style='font-size:xx-large'>対応依頼</p></div>
    <div><p style='text-align:right'><a href="message_form.php">新規作成</a></p><br></div>
    <table class="table table-striped">
        <thead>
          <tr>
            <th scope="col" style='text-align:center;  background-color:lightsteelblue'></th>
        ...
        <tbody>
          ...
          <?php while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) : ?>
            <tr data-href="message_detail.php?id=<?php echo $row['id'];?>">
              <td  style='text-align:center; <?php if($row["status"]==0){echo("color:brown;'>");}else{echo("'>");} ?><?php if($row["status"]==0){echo("未読");}else{echo ("既読");} ?></td>
              <td  style='text-align:center;'><?php echo $row["user_id"]; ?></td>
              <td style='text-align:center;'><?php echo $row["created_at"]; ?></td>
              <td><?php echo $row["title"]; ?></td>
              <td  style='text-align:center;'><a href="message_detail.php?id=<?php echo $row['id'];?>">編集</a></td>
              <td style='text-align:center;'><a href="message_delete.php?id=<?php echo $row['id'];?>">削除</a></td>
            </tr>
          <?php endwhile; ?>
        </tbody>
      </table>
